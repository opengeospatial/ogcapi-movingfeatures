...
Collection Information is the set of metadata that describes a single collection.
...
If a `mFeatureID` is not specified, the query will return a list of the available mFeatureID’s. This behavior is specified in OGC API-Features. all other parameters for use with the Items query are defined by OGC API-Features.
