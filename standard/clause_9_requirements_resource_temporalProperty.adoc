...
The *TemporalProperty* resource returned to the response can be limited using the parameters <<leaf-section,`leaf`>> and <<subTemporalValue-section,`subTemporalValue`>>.
...
The schema for the temporal property object presented in this clause is an extension of the link:https://docs.opengeospatial.org/is/19-045r3/19-045r3.html#tproperties[*_ParametricValues_* Object] defined in <<OGC-MF-JSON,MF-JSON>>.
