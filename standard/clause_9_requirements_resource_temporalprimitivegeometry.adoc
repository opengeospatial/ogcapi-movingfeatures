...
The schema for the TemporalPrimitiveGeometry presented in this clause is an extension of the `TemporalPrimitiveGeometry Object` defined in link:https://docs.opengeospatial.org/is/19-045r3/19-045r3.html#tprimitive[MF-JSON standard].
...
|*_id_*                 |M  |A unique identifier to the temporal geometry.
...
. Issue a `DELETE` request on `+{root}+/collections/+{collectionId}+/items/+{mFeatureId}+/tgeometry/+{tGeometryId}+` path
...
The list of valid values for `+{tGeometryId}+` is provided in the <<resource-temporalGeometry-section,`+{root}+/collections/+{collectionId}+/items/+{mFeatureId}+/tgeometry`>> GET response.
