...
The `subTrajectory` query parameter is used to select a subset of <<resource-temporalGeometrySequence-section,*TemporalGeometrySequence*>> of each <<resource-movingfeature-section,*MovingFeature*>> in the *MovingFeatures*, for the specified time interval.
...
Also, the `interpolation` property in the response shall be the same as the original temporal primitive geometry.
