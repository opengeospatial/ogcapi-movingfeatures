...
The sequence of <<resource-temporalPrimitiveGeometry-section,*TemporalPrimitiveGeometry*>> object returned to the response can be limited using the query parameters <<limit-section,`limit`>>, <<bbox-section,`bbox`>>, <<datetime-section,`datetime`>>, <<leaf-section,`leaf`>>, and <<subtrajecotry-section,`subTrajectory`>>.
