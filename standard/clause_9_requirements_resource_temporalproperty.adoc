...
. A create operation posts a new temporal value object to the <<resource-tproperties-collection-section,`TemporalPropertyCollection`>> that specified by <<tproperties-identifier,{tPropertyName}>>.
...
|*_values_*             |M |A sequence of dynamic value, having the same number of elements as "datetimes".
