...
. A retrieve operation returns a `TemporalProperty` resource which is included in the <<resource-temporalProperties-section,`TemporalProperties`>> that specified by <<tproperties-identifier,++{tPropertyName}++>>.The `TemporalProperty` resource returned to the response can be limited using the <<limit-section,`limit`>>, <<datetime-section,`datetime`>>, and <<leaf-section,`leaf`>> parameters.
. A create operation posts a new temporal value object to the <<resource-temporalProperties-section,`TemporalProperties`>> that specified by <<tproperties-identifier,++{tPropertyName}++>>.
...
|*_values_*             |M |A sequence of <<temporalValue-properties-table, temporal value>>
...
This operation targeted the new temporal values object defined in <<temporalValue-properties-table>>.
...
The following example adds a new feature (<<temporalValue-properties-table,TemporalValue>> object) to the feature created by <<tproperties-post-example,Creating a New TemporalProperty Object Example>>.
...
.An Example of Creating a New TemporalValue Object:
