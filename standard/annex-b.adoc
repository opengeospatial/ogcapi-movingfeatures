...
=== Introduction
The Abstract Test Suite (ATS) is a compendium of test assertions applicable to implementations of the OGC API — Moving Features. An ATS provides a basis for developing an Executable Test Suite to verify that the implementation under test conforms to all the relevant functional specifications.

The abstract test cases (assertions) are organized into test groups that correspond to distinct conformance test classes defined in the OGC API — Moving Features Standard.
