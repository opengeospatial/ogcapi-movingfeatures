...
IF a query parameter `datatime` is empty: Verify that a time-to-velocity curve is correctly returned according to the specified TemporalGeometry resource by {tgeometryId}.
|===