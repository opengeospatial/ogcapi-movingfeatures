...
|Requirement    | <<req_mf-tproperties-collection-response-get, /req/movingfeatures/tproperties-collection-get-success>>
...
3. Validate the `type` property is present and its value is one of the predefined value (i.e., one of 'TBoolean', 'TText', 'TInteger', 'TReal', and 'TImage') +
...
8. Validate the TemporalPropertyCollection resource for all supported media types using the resources and tests identified in <<temporalproperties-schema>>
|===
...
.Schema and Tests for TemporalPropertyCollection content
